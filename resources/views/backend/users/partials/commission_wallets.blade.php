...
<div id="fund_wallet" class="tab-pane active">
    <div class="panel-body">
        <h3 class="title">Commission wallet History <a class="btn btn-success pull-right" data-toggle="modal" data-target="#fundWallet"><i class="fa fa-edit"></i> Edit Commission Wallet</a></h3>
        <table class="table table-stripped">
            <thead>
                ...
                @if(isset($commission_wallets) && count($commission_wallets)>0)
                    ...
                    <tfoot>
                        <tr align="right">
                            <td colspan="6">{!! $commission_wallets->render('vendor.default_paginate') !!}</td>
                        </tr>
                    </tfoot>
                @else
                <tbody >
...
<div id="fundWallet" class="modal fade" role="dialog">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            {!! Form::open(['route'=>'commission-wallet-history.store','method'=>'post','class'=>'form-vertical','id'=>'userall_wallets','autocomplete'=>'false']) !!}
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">Edit Commission Wallet [{{$user->name}}]</h4>
            </div>
            <div class="modal-body">
                <div class="form-group">
                    <input type="hidden" name="user_id" value="{{$user->id}}" />
                    <label>Amount</label> 
                     {!! Form::text('amount',old('amount',($user->userwallet!=null?$user->userwallet->commission_wallet:"0")),['min'=>'0','class'=>'form-control','placeholder'=>'Enter Amount']) !!}
